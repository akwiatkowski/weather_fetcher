...
describe WeatherFetcher::Provider::OpenWeatherMapForecast do
  before :each do
    @defs = cities_defs
    @klass = WeatherFetcher::Provider::OpenWeatherMapForecast
  end
  ...
  it "simple fetch" do
    ...
    weathers.each do |w|
      expect(w.provider).to eq(WeatherFetcher::Provider::OpenWeatherMapForecast.provider_name)
    end
